...
angree = 4
WG3Y = [WGWzuoY]
WG3Z = [[0]*len(i) for i in WGWzuoY]
WG3X = [WGWzuoX]
WGx = []
WGy = []
WGz = []
for i in range(angree):
    yj = []  # 每个角的总轮廓y
    zj = []  # 每个角的总轮廓z
    for j in range(len(WGWzuoY)):
        r = WGWzuoY[j]
        yk = []  # 每层轮廓y
        zk = []  # 每层轮廓z
...
# 生成节点编号及坐标
path = "E:\\CADWINDGX\\data3d.txt"  # 文件路径
if os.path.exists(path):  # 如果文件存在
    os.remove(path)
data = open("E:\\CADWINDGX\\data3d.txt", 'w+')
print('*Node', file=data)
for d in range(angree):
    dd = 10000*d
    for i in range(len(WGWzuoX)):
        ...
        if len(WGWzuoX[i]) < n0:  # 防止节点重合
            ...
            for j in range(1, len(WGWzuoX[i])):
                nn += 1
                print(nn + nm + dd, ',  ', WGx[d][i][j], ',  ', WGy[d][i][j],  ',  ', WGz[d][i][j], file=data)
        else:
            nn = 0
...
for d in range(angree):
    ...
    for i in range(len(WGWzuoX)):
        if len(WGWzuoX[i]) < n0:  # 防止节点重合
            t = list(range(dd+100 * i + (n0 - len(WGWzuoX[i])) + 2, dd+100 * (i + 1) + 1))
            nnum.append(t)
        else:
            nnum.append(list(range(dd+100 * i + 1, dd+100 * (i + 1) + 1)))
    nnum3d.append(nnum)
                    ...
                    if nnumi[j] - 100*k in nnum[i - k]:
                        if nnumi[j] - 100*(k-1)+1 in nnum[i-k+1] and k>1:
                            print(en, ',   ', nnumi[j] - 100 * k, ',   ', nnumi[j] - 100*(k-1)+1,
                                      ',   ', nnumi[j] + 1, ',   ', nnumi[j],
                                      ',   ', nnum2i[j] - 100 * k, ',   ', nnum2i[j] - 100*(k-1)+1,
                                      ',   ', nnum2i[j] + 1, ',   ', nnum2i[j],
                                  file=data)
                            en += 1
                            break
                        else:
                            print(en, ',   ', nnumi[j] - 100*k, ',   ', nnumi[j] - 100*k+1,
                                      ',   ', nnumi[j] + 1, ',   ', nnumi[j],
                                      ',   ', nnum2i[j] - 100 * k, ',   ', nnum2i[j] - 100 * k + 1,
                                      ',   ', nnum2i[j] + 1, ',   ', nnum2i[j],
                                  file=data)
                            en += 1
                            break
...
for d in range(angree - 1):
    nnum = nnum3d[d]  # 相邻角度的节点编号
    nnum2 = nnum3d[d + 1]  # 相邻角度的节点编号
    for i in range(1, len(nnum)):
        nnumi = nnum[i]
        nnum2i = nnum2[i]
        if len(nnumi) < n0:  # 扩孔，有三角形单元
            print(en, ',   ', nnumi[0], ',   ', nnumi[0] - 101, ',   ', nnumi[0] - 100,
                      ',   ', nnum2i[0], ',   ', nnum2i[0] - 101, ',   ', nnum2i[0] - 100,
                  file=data)
            en += 1
