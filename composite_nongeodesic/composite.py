...
def draw_b_spline(k, X, Y,nn):
    n = len(X)
    #    nodeVector = list(range(len(X) - 2))
    #    nodeVector = [0] * k + nodeVector + [nodeVector[-1]] * k
    ...
    func0 = interpolate.interp1d(rx, ry, kind='cubic')
    rxnew = X
    rynew = func0(rxnew)
    ...
    # LLL = [0] * len(rx)
    # xy = 0
    # for i in range(len(rx) - 1):  # 计算总长度
    #     lll = ((rx[i + 1] - rx[i]) ** 2 + (ry[i + 1] - ry[i]) ** 2) ** 0.5
    #     xy += lll
    #     LLL[i + 1] = xy
    # S = LLL[-1]
    # a = S / (nn - 1)
    ...
    # nrx = [0] * nn
    # nry = [0] * nn
    # for i in range(nn - 1):  # 按比例生成均匀点列表
    #
    #     for j in range(len(rx) - 1):
    #         if (xy >= LLL[j]) & (xy < LLL[j + 1]):
    #             nrx[i + 1] = (rx[j + 1] - rx[j]) * (xy - LLL[j]) / (LLL[j + 1] - LLL[j]) + rx[j]
    #             nry[i + 1] = (ry[j + 1] - ry[j]) * (xy - LLL[j]) / (LLL[j + 1] - LLL[j]) + ry[j]
    #             break
    #     xy += a
    # nrx[0] = rx[0]
    # nry[0] = ry[0]
...
def func1(y,z):
    ...
    r = R0 * (b**2 - (z-b)**2 )**0.5/b
    r1 = -R0 *(z-b)/(b*(b**2-(z-b)**2)**0.5)
    r11 = -R0*b/(b**2-(z-b)**2)**(3.0/2)
...
def sanciyangtiaohouduyuce(x_coordinate,b_fiber,tP,nR):#nR螺旋缠绕层数，tP单层纱带厚度
    thick_list=[]
    thick_list1=[]
    thick_list2=[]
    r0=min(x_coordinate)
    ...
    int1,err1=integrate.quad(lambda r:2*pi*r*mR*nR/pi*np.arccos(r0/r)*tP,r0,rb)#quad r0到rb求积分
    ...
    B=np.array([[b1],[b2],[b3],[b4]],dtype=float)#强制转化成数组
...
def mian(L_style,L_segmentationNum,R_segmentationNum,rotationAngle,computing_method,material_property,
        pieceNumber,approximateSize,L_ellipticalheight,R_ellipticalheight,
        modelName,assemblyName,L_setName,R_setName):
    
    L_tables=[]
    R_tables=[]
    L_helixThickness,R_helixThickness=[],[]
    
    L_helixThickness_Friction,R_helixThickness_Friction=[],[]#考虑带有滑移系数的缠绕
    
    L_hoopThickness,R_hoopThickness=[],[]
    L_strengthThickness,R_strengthThickness=[],[]
    L_reamingThickness,R_reamingThickness=[],[]
    
    L_reamingThickness_Friction,R_reamingThickness_Friction=[],[]
    ...
    L_reamingRadius=[]
    R_reamingRadius=[]
    L_helix_slipcoefficient=[]
    R_helix_slipcoefficient=[]
    ...
    L_hoop_transition=[]
    R_hoop_transition=[]
# x[0]缠绕形式,x[1]角度,x[2]补强形式，x[3]补强区间上,x[4]补强区间下,x[5]扩孔半径.x[6]滑移系数,x[7]厚度,x[8]纱线宽度，x[9]位置,x[10]材料名
    for x in L_style:
        if x[10] == "up":
            if x[0] == 'helix':
                L_tables.append('helix')
                L_helixThickness.append(x[7])
                L_materialname.append(x[11])
                L_fiberwidth.append(x[8])
            elif x[0] == 'helixReaming':
                L_tables.append('helixReaming')
                #L_reamingAngle.append(x[1])
                L_reamingRadius.append(x[5])
                L_reamingThickness.append(x[7])
                L_materialname.append(x[11])
                L_fiberwidth.append(x[8])
            elif x[0] == 'strengthening':
                L_tables.append('strengthening')
                L_strengthLocation.append((x[3],x[4]))
                L_strengthThickness.append(x[7])
                L_materialname.append(x[11])
                L_strengthtype.append(x[2])
                if x[5] == 'windReinforce':
                    L_strengthangle.append(0)
                else:
                    L_strengthangle.append(x[1])
                L_strengthfiber.append(x[8])
                
            elif x[0] == 'hoop':
                L_tables.append('hoop')
                L_hoopThickness.append(x[7])
                L_materialname.append(x[11])
                L_hoop_transition.append(x[9])
            elif x[0]=='helix(Friction)':
                L_tables.append('helix(Friction)')
                L_helixThickness_Friction.append(x[7])
                L_helix_slipcoefficient.append(x[6])
                L_materialname.append(x[11])
                L_fiberwidth.append(x[8])
            elif x[0]=='helixReaming(Friction)':
                L_tables.append('helixReaming(Friction)')
                L_reamingThickness_Friction.append(x[7])
                L_helix_slipcoefficient.append(x[6])
                L_reamingRadius.append(x[5])
                L_materialname.append(x[11])
                L_fiberwidth.append(x[8])
            ...
            if x[0] == 'helix':
                ...
                R_helixThickness.append(x[7])
                R_materialname.append(x[11])
                R_fiberwidth.append(x[8])
            elif x[0] == 'helixReaming':
                R_tables.append('helixReaming')
                #R_reamingAngle.append(x[1])
                R_reamingRadius.append(x[5])
                R_reamingThickness.append(x[7])
                R_materialname.append(x[11])
                R_fiberwidth.append(x[8])
            elif x[0] == 'strengthening':
                R_tables.append('strengthening')
                R_strengthLocation.append((x[3],x[4]))
                R_strengthThickness.append(x[7])
                R_materialname.append(x[11])
                R_strengthtype.append(x[2])
                if x[5] == 'windReinforce':
                    R_strengthangle.append(0)
                else:
                    R_strengthangle.append(x[1])
                R_strengthfiber.append(x[8])
                
            elif x[0] == 'hoop':
                R_tables.append('hoop')
                R_hoopThickness.append(x[7])
                R_materialname.append(x[11])
                R_hoop_transition.append(x[9])
            elif x[0]=='helix(Friction)':
                R_tables.append('helix(Friction)')
                R_helixThickness_Friction.append(x[7])
                R_helix_slipcoefficient.append(x[6])
                R_materialname.append(x[11])
                R_fiberwidth.append(x[8])
            elif x[0]=='helixReaming(Friction)':
                R_tables.append('helixReaming(Friction)')
                R_reamingThickness_Friction.append(x[7])
                R_helix_slipcoefficient.append(x[6])
                R_reamingRadius.append(x[5])
                R_materialname.append(x[11])
                R_fiberwidth.append(x[8])
    ...
    num1=L_segmentationNum
    num2=R_segmentationNum
#2021.10.9增加算法选择和材料属性赋予computing_method,material_property
    ...
    # if typeReinforce =="windReinforce":
        # type_reinforce="windReinforce"##"pufangbuqiang" #
    # else:
    ...
    material_section=[]#用于获取不同材料并创建截面属性2021.8.25
    for name in cailiao:
        material_section.append(name)
    for i in range(len(material_section)):
        m.HomogeneousSolidSection(name='Section-'+str(material_section[i]), material=material_section[i], thickness=None)

    hoop_xiedegree=[]#用于存储环向层过渡区域的坐标，2021.8.31
    ...
    if material_property=='create composite layup':
        for edge in edgeset.edges:
            try:
                lineLength=edge.getSize(printResults=False)
                partnum=num1   #封头切片num
                for i in range(1,partnum):#（从1开始到倒数第二个点，没有包含顶点）
                    tonum=1.0/partnum*i
                    curpoint = edge.getCurvature(parameter=tonum)#返回值是包含 'evaluationPoint', 'curvature', 'radius', 'tangent'的字典
                    evaPoint=curpoint['evaluationPoint']#其中“evaluationPoint”指定了所计算曲率的位置坐标。
                    tabgent=curpoint['tangent']
                    tangentpoint[(evaPoint[0],evaPoint[1])]=tabgent#将键和值关联起来。没有用到
                    curve_x.append(evaPoint[0])
                    curve_y.append(evaPoint[1])
                curveLineVertices=edge.getVertices()#获取封头曲线上下顶点的索引序列
                topindex,botindex=curveLineVertices[0],curveLineVertices[1]
                toppoint=ver[topindex].pointOn[0]#ver获得顶点索引的坐标，((90.0, 225.0, 0.0),)
                botpoint=ver[botindex].pointOn[0]
               
                spline=zip(curve_x,curve_y)#将curve_x和curve_y对应数打包成列表
                spline.insert(0,(toppoint[0],toppoint[1])) #在spline列表第一个位置插入上顶点
                spline.insert(-1,(botpoint[0],botpoint[1]))#在spline列表最好位置的前一位插入下顶点，所以导致了点的顺序发生改变
                tangentpoint[(toppoint[0],toppoint[1])]=(0,1,1)
                tangentpoint[(botpoint[0],botpoint[1])]=(0,1,2)
                spline.sort(key=(lambda x: x[1]))  #sort对列表按照第二维升序排列
        ...
        incoordinate=tangentpoint.keys()#将tangentpoint中的键信息赋予incoordinate，这里的键是坐标
        incoordinate.sort(key=(lambda x: x[1])) #key=lambda 元素: 元素[字段索引]，按照第二维进行升排序
        incoordinate.reverse()#降序排列,右封头就不需要降序排列
        ...
        L = min(y_coordinate) #筒身长度一半,封头上最小y坐标
        global R0
        R0 = max(x_coordinate)
        ...
        L_helixThickness_FrictionCount = 0#2021.3.26
        L_reamingThickness_FrictionCount = 0
        L_helix_slipcoefficientCount = 0
        L_reamingRadiusCount = 0
        ...
        L_puceng=[]
    ################2021.8,18
        L_buqiangangle_count=0
        L_typeReinforce_count=0
        L_fiberwidth_count=0    
        b=L_ellipticalheight
        ##############2021.9.7
        L_hoop_transition_count=0
        windtype_count=0
        for windtype in L_tables:
            if windtype == 'helix':
                if L_hoop_transition_count==0:#2021.9.7增加可变的环向过渡长度
                    xiedegree=1
                    y2=L-xiedegree            
                    leftAcc=y2
                else:
                    xiedegree=L_hoop_transition[L_hoop_transition_count-1]
                    y2=L-xiedegree 
                    leftAcc=y2
                b_fiber=L_fiberwidth[L_fiberwidth_count]
                L_fiberwidth_count+=1
                fengtoui=cankaolunkuo#内轮廓等于参考轮廓
                #更新实际坐标
                x_coordinate=[]
                y_coordinate=[]
                for i in range(len(fengtoui)):
                    x_coordinate.append(fengtoui[i][0])#第i行第1列
                    y_coordinate.append(fengtoui[i][1])
                tP = L_helixThickness[L_helixThicknessCount]#索引缠绕层某一序列对应的厚度。
                ...
                #################
                if computing_method=='B-spline':
                    x = xvalue
                    y = yvalue
                    rxnew, rynew = draw_b_spline(3, x, y,num1)
                    #rxnew=rxnew.tolist()
                    rynew=rynew.tolist()
                else:
                    rxnew=xvalue
                ...
                #筒身
                newRi=L_thickness  # L_thickness=R0
                L_thickness+=tP*2
                newRo=newRi+tP*2
                pentagon=0#五边形，用于判断螺旋层过渡区域是否需要切分
                if windtype_count==0:
                    ...
                    helix_point1=(newRo,y2)
                else:
                    if L_tables[windtype_count-1]!='hoop':
                        line1=s1.Line(point1=(newRi,y2), point2=(newRi,0))
                        line2=s1.Line(point1=(newRo,y2), point2=(newRo,0))
                        line1=s1.Line(point1=(newRi,y2), point2=(fengtoui[-1][0],fengtoui[-1][1]))
                        line2=s1.Line(point1=(newRo,y2), point2=(fengtouo[-1][0],fengtouo[-1][1]))
                        ...
                        # hoop_num=0
                        # for i in range(1,windtype_count):
                            # if L_tables[i]=='hoop':
                                # hoop_num+=1
                        if windtype_count-1>=1:
                            line1=s1.Line(point1=(newRi,y2), point2=(newRi,0))
                            line2=s1.Line(point1=(newRo,y2), point2=(newRo,0))
                            line1=s1.Line(point1=hoop_point1, point2=(fengtoui[-1][0],fengtoui[-1][1]))
                            line2=s1.Line(point1=(newRi,y2), point2=hoop_point1)
                            line3=s1.Line(point1=(newRo,y2), point2=(fengtouo[-1][0],fengtouo[-1][1]))
                            helix_point1=(newRo,y2)
                            pentagon=1
                        else:
                            line1=s1.Line(point1=(newRi,y2), point2=(newRi,0))
                ...
                #更新参考轮廓
                cankaolunkuo=fengtouo #内轮廓加一个厚度之后的轮廓
                ...
                for i in range(len(fengtouo)-1):
                    ...
                    #用findAt需要注意的是一定要让给出的点坐标仅属于一个几何体，不要提供公共点的坐标，否则findAt函数会给出不确定的答案。
                ...
                pickface=f1.findAt(coordinates=(0.5*(newRi+newRo), y2/2.0, 0.0))#找到需要拆分的面
                ...
                for k in range(1,len(fengtoui)):#将内外层对应点连接，构造草图切分
                    s.Line(point1=fengtoui[k],point2=fengtouo[k])
                p1.PartitionFaceBySketch(sketchUpEdge=pickedge, faces=pickface,sketch=s)#用草图创建拆分面
                s.unsetPrimaryObject()
                del m.sketches['__profile__']#删除临时草图
                #封头处切分
                ...
                for k in range(1,len(fengtoui)):
                    c=p1.cells
                    pickedEdges =(e1.findAt(coordinates=(0.5*(fengtoui[k][0]+fengtouo[k][0]), 0.5*(fengtoui[k][1]+fengtouo[k][1]), 0.0)),)#获得xy平面上内外层对应点连线的中点。
                ...
                ######2021.8.30
                ds=p1.datums
                PL1=p1.DatumPlaneByPrincipalPlane(principalPlane=XZPLANE, offset=leftAcc)
                c = p1.cells
                p1.PartitionCellByDatumPlane(cells=c,datumPlane=ds[PL1.id])
                ######################
                f_all=p1.faces
                face_coordinate=[]
                face_coordinate_xiedegree=[]
                for face in f_all:
                    centercoordinate=face.getCentroid()
                    if centercoordinate[0][2] ==0.0 and centercoordinate[0][0]>0 and centercoordinate[0][1]>L :
                        face_coordinate.append(centercoordinate[0])
                    elif centercoordinate[0][2] ==0.0 and centercoordinate[0][0]>0 and centercoordinate[0][1]<y2:
                        face_coordinate.append(centercoordinate[0])
                    elif centercoordinate[0][2] ==0.0 and centercoordinate[0][0]>0 and centercoordinate[0][1]>y2 and centercoordinate[0][1]<L:
                        face_coordinate_xiedegree.append(centercoordinate[0])
                    else:
                        continue
                
                face_coordinate.sort(key=(lambda x: x[0]))#按照x升序排列
                count=0
                list_angle=[]
                ...
                for k in face_coordinate:
                    count=count+1
                    pickcells=c.findAt(((k[0], k[1], k[2]),))  #对应（x,y,z）
                ...
                for k in face_coordinate_xiedegree:
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='L_set-'+str(L_lxcount)+'-'+str(count)+'-xiedegree',cells=pickcells)
                ...
                if pentagon==1:
                    ds=p1.datums
                    PL1_1=p1.DatumPlaneByPrincipalPlane(principalPlane=XZPLANE, offset=hoop_point1[1])
                    c = p1.cells
                    p1.PartitionCellByDatumPlane(cells=c,datumPlane=ds[PL1_1.id])
                ########2021.8.27
                center=(0,0,0)
                cs1=luoxuanceng.cells
                c1=cs1.getByBoundingSphere(center=center,radius=4*abs(L))
                L_puceng.append(c1)

                SDcoordinates=(newRo*cos(0.5*rotangle*todegree),y2/2.0,-newRo*sin(0.5*rotangle*todegree))#找到筒身最外侧的点，最后用于指派堆叠方向
                b+=tP
                R0+=tP##2021.8.28
                windtype_count+=1#2021.9.8记数缠绕形式
                
                
                                
                # #########指派法对材料层进行切分，然后对上下两层分别指派角度
                # #画切分线
                # f1, e1, d2 = p1.faces, p1.edges, p1.datums
                # pickface=f1.findAt((face_coordinate[0],))
                # for i in range(1,len(fengtoui)-1):
                    # faces1=f1.findAt((face_coordinate[i],))
                    # pickface+=faces1
                ...
                # p1.projectReferencesOntoSketch(sketch=s, filter=COPLANAR_EDGES)
                # for i in range(len(fengtoui)-1):
                    # s.Line(point1=((fengtoui[i][0]+fengtouo[i][0])/2.0,(fengtoui[i][1]+fengtouo[i][1])/2.0),point2=((fengtoui[i+1][0]+fengtouo[i+1][0])/2.0,(fengtoui[i+1][1]+fengtouo[i+1][1])/2.0))
                ...
                # for i in range(len(fengtoui)-1):
                    # c=p1.cells
                    # pickedEdges =(e1.findAt(coordinates=(((fengtoui[i][0]+fengtouo[i][0])/2.0+(fengtoui[i+1][0]+fengtouo[i+1][0])/2.0)/2.0,((fengtoui[i][1]+fengtouo[i][1])/2.0+(fengtoui[i+1][1]+fengtouo[i+1][1])/2.0)/2.0, 0.0)),)#这些线事先已经连上，才能用findat找到。
                    # p1.PartitionCellBySweepEdge(sweepPath=e1.findAt(coordinates=(fengtouo[-1][0]*cos(0.5*rotangle*todegree), L, -1*fengtouo[-1][0]*sin(0.5*rotangle*todegree))), cells=c, edges=pickedEdges)
                # # ##
                # #####找到上下两层面内的点，创建集合
                # upface_point=[]
                # downface_point=[]
                # for i in range((len(fengtoui)-1)):
                    # downface_point.append(((fengtoui[i][0]+fengtoui[i+1][0])/2.0+0.01,(fengtoui[i][1]+fengtoui[i+1][1])/2.0+0.01,0))
                # for i in range((len(fengtouo)-1)):
                    # upface_point.append(((fengtouo[i][0]+fengtouo[i+1][0])/2.0-0.01,(fengtouo[i][1]+fengtouo[i+1][1])/2.0-0.01,0))
                # count=0
                # for k in upface_point:
                    # count=count+1
                    # pickcells=c.findAt(((k[0], k[1], k[2]),))
                    # p1.Set(name='L_set-'+str(L_lxcount)+'-'+str(count)+'up',cells=pickcells)
                # count=0 
                # for k in downface_point:
                    # count=count+1
                    # pickcells=c.findAt(((k[0], k[1], k[2]),))
                    # p1.Set(name='L_set-'+str(L_lxcount)+'-'+str(count)+'down',cells=pickcells)
                # ##########################
                ...
                r=L_reamingRadius[L_reamingRadiusCount]
                L_reamingRadiusCount+=1
                ...
                #扩孔的实际内轮廓
                fengtoui=kfengtoui[indexnum:]#扩孔位置的x到后面所有数
                #右边尖孔
                ...
                if windtype_count==0:
                    ...
                    if L_tables[windtype_count-1]!='hoop':
                        ...
                        if windtype_count-1>=1:
                            ...
                            line3=s1.Line(point1=(newRo,y2), point2=(fengtouo[-1][0],fengtouo[-1][1]))
                            helix_point1=(newRo,y2)
                            pentagon=1#五边形，用于判断螺旋层过渡区域是否需要切分
                        else:
                            line1=s1.Line(point1=(newRi,y2), point2=(newRi,0))
                            line2=s1.Line(point1=(newRo,y2), point2=(newRo,0))
                            line1=s1.Line(point1=(newRi,y2), point2=(fengtoui[-1][0],fengtoui[-1][1]))
                ...
                for k in range(1,len(fengtoui)):
                    c=p1.cells
                    pickedEdges =(e1.findAt(coordinates=(0.5*(fengtoui[k][0]+fengtouo[k][0]), 0.5*(fengtoui[k][1]+fengtouo[k][1]), 0.0)),)#这些线事先已经连上，才能用findat找到。
                    p1.PartitionCellBySweepEdge(sweepPath=e1.findAt(coordinates=(fengtouo[-1][0]*cos(0.5*rotangle*todegree), L, -1*fengtouo[-1][0]*sin(0.5*rotangle*todegree))), cells=c, edges=pickedEdges)
                #设置集####且根据中心点坐标计算对应缠绕角度
                ...
                for k in face_coordinate_xiedegree:
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='L_set-'+str(L_lxcount)+'-'+str(count)+'-xiedegree',cells=pickcells)
                all_angle.append(list_angle)  #总角度中添加一个角度列表，方便后期调用
                #赋予角度
                all_count.append(count)
                ...
                # s.Line(point1=(left_point[0],left_point[1]),point2=((fengtoui[0][0]+fengtouo[0][0])/2.0,(fengtoui[0][1]+fengtouo[0][1])/2.0))
                # for i in range(len(fengtoui)-1):
                    # s.Line(point1=((fengtoui[i][0]+fengtouo[i][0])/2.0,(fengtoui[i][1]+fengtouo[i][1])/2.0),point2=((fengtoui[i+1][0]+fengtouo[i+1][0])/2.0,(fengtoui[i+1][1]+fengtouo[i+1][1])/2.0))
                # p1.PartitionFaceBySketch(sketchUpEdge=pickedge, faces=pickface,sketch=s)
                # s.unsetPrimaryObject()
                ...
                # upface_point=[]
                # downface_point=[]
                # downface_point.append(((fengtoui[0][0]+left_point[0])/2.0+0.01,(fengtoui[0][1]+left_point[1])/2.0+0.01,0))
                # upface_point.append(((fengtouo[0][0]+left_point[0])/2.0-0.01,(fengtouo[0][1]+left_point[1])/2.0-0.01,0))
                # for i in range((len(fengtoui)-1)):
                    # downface_point.append(((fengtoui[i][0]+fengtoui[i+1][0])/2.0+0.01,(fengtoui[i][1]+fengtoui[i+1][1])/2.0+0.01,0))
                # for i in range((len(fengtouo)-1)):
                    # upface_point.append(((fengtouo[i][0]+fengtouo[i+1][0])/2.0-0.01,(fengtouo[i][1]+fengtouo[i+1][1])/2.0-0.01,0))
                ...
                b_fiber=L_fiberwidth[L_fiberwidth_count]
                L_fiberwidth_count+=1
                fengtoui=cankaolunkuo
                x_coordinate=[]
                y_coordinate=[]
                for i in range(len(fengtoui)):
                    x_coordinate.append(fengtoui[i][0])
                    y_coordinate.append(fengtoui[i][1])
                tP = L_helixThickness_Friction[L_helixThickness_FrictionCount]
                L_helixThickness_FrictionCount+=1
                thick_list,r0,R,alpha=sanciyangtiaohouduyuce(x_coordinate,b_fiber,tP,nR)
                s1 = m.ConstrainedSketch(name='L_helixLayer'+str(L_lxcount-L_buqiang), sheetSize=200.0)
                cline=s1.ConstructionLine((0,10),(0,-10))
                s1.assignCenterline(line=cline)
                ...
                r0=min(x_coordinate)
                h=(b**2-b**2*r0**2/R0**2)**0.5 #极孔高度
                u=L_helix_slipcoefficient[L_helix_slipcoefficientCount]#滑移系数
                L_helix_slipcoefficientCount+=1
                ...
                RG4=odeint(func1,ff0,z1)#求解微分方程，结果类型为数组           
                alphaRadian = RG4[:,0] #弧度制缠绕角
                alphaAngle=RG4[:,0]/pi*180 #角度制缠绕角
                list_angle=alphaAngle.tolist()#将数组转换成列表类型存储
            ...
            elif windtype == 'helixReaming(Friction)': 
                if L_hoop_transition_count==0:#2021.9.7增加可变的环向过渡长度
                    xiedegree=1
                    y2=L-xiedegree            
                    leftAcc=y2
                ...
                tP = L_reamingThickness_Friction[L_reamingThickness_FrictionCount]#索引缠绕层某一序列对应的厚度。
                L_reamingThickness_FrictionCount+=1
                ...
                type_reinforce=L_strengthtype[L_buqiang]
                buqiangangle = L_strengthangle[L_buqiang]
                ...
                r0=left_point[0]
                solvek1=solvek[indexnum_i:indexnum_j+1]#solvek=cankaolunkuo
                ...
                #更新参考轮廓
                if x2 <= L:
                    cankaolunkuo1=kfengtoui[:indexnum_i]+fengtouo #极孔到补强区间上+补强区域
                    cankaolunkuo1.append(fengtoui[-1])
                    cankaolunkuo=cankaolunkuo1+kfengtoui[indexnum_j:] #fengtoui=kfengtoui[indexnum_i:indexnum_j+1]
                else:
                ...
                for face in f_all:
                    ...
                    if centercoordinate[0][2] ==0.0 and centercoordinate[0][0]>0:
                        face_coordinate.append(centercoordinate[0])
                #face_coordinate.sort(key=(lambda x: x[1]), reverse=True)
                ...
                for i in range(len(fengtouo)-1):
                    side1Faces11 = f.findAt(((((fengtouo[i][0]+fengtouo[i+1][0])/2)*cos(0.5*rotangle*todegree),(fengtouo[i][1]+fengtouo[i+1][1])/2,-(fengtouo[i][0]+fengtouo[i+1][0])/2*sin(0.5*rotangle*todegree)), ))
                    p1.Surface(side1Faces=side1Faces11,
                               name='L_surfStrengtheningTop'+str(L_lxcount)+'-'+str(i+2))
                ...
                for k in range(0,len(fengtoui)-1):
                    s.Line(point1=fengtoui[k],point2=fengtouo[k])
                ...
                face_coordinate=[]
                for face in f_all:
                    centercoordinate=face.pointOn#不用getCentroid()，后者点不一定在面上
                    if centercoordinate[0][2] ==0.0 and centercoordinate[0][0]>0:#z为0,同时x大于0.因为转180度时，会出现两个面的z都为0
                        face_coordinate.append(centercoordinate[0])
                face_coordinate.sort(key=(lambda x: x[1]), reverse=True)
                
                f1 = p1.faces
                # 左边尖角
                pickface = f1.findAt(((0.5*(fengtouo[0][0]+left_point[0])*cos(0.5*rotangle*todegree),
                           0.5*(fengtouo[0][1]+left_point[1]),
                           -0.5*(fengtouo[0][0]+left_point[0])*sin(0.5*rotangle*todegree)), ))
                p1.Surface(side1Faces=pickface, name='L_surfStrengtheningTop'+str(L_lxcount)+'-'+str(1))
                # 右边尖角
                pickface = f1.findAt(((0.5 * (fengtouo[-1][0] + fengtoui[-1][0]) * cos(
                    0.5 * rotangle * todegree),
                                       0.5 * (fengtouo[-1][1] + fengtoui[-1][1]),
                                       -0.5 * (fengtouo[-1][0] + fengtoui[-1][0]) * sin(
                                           0.5 * rotangle * todegree)),))
                p1.Surface(side1Faces=pickface,
                           name='L_surfStrengtheningTop'+str(L_lxcount)+'-'+str(len(fengtoui)))
                ...
                xiedegree=L_hoop_transition[L_hoop_transition_count]
                L_hoop_transition_count+=1
                y2=L-xiedegree 
                leftAcc=y2
                ...
                if windtype_count==0:
                    ...
                    line1=s2.Line(point1=(tnewRi,y2), point2=(tnewRi,0))
                    line2=s2.Line(point1=(tnewRo,y2), point2=(tnewRo,0))
                    line1=s2.Line(point1=helix_point1, point2=(tnewRi,y2))
                    line2=s2.Line(point1=helix_point1, point2=(tnewRo,y2))
                    hoop_point1=helix_point1
                ...
                for face in f_all:
                    ...
                    if centercoordinate[0][2] ==0.0 and centercoordinate[0][0]>0 and centercoordinate[0][1]>L :
                        ...
                        continue
                face_coordinate.sort(key=(lambda x: x[0]))#按照y降序排列，按照x会出现问题
                for k in face_coordinate:
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='L_set-'+str(L_hxcount),cells=pickcells)
                for k in face_coordinate_xiedegree:
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='L_set-'+str(L_hxcount)+'-xiedegree',cells=pickcells)
                ########2021.8.27
                center=(0,0,0)
                cs1=huanxiangceng.cells
                c1=cs1.getByBoundingSphere(center=center,radius=4*abs(L))
                L_puceng.append(c1) 
                SDcoordinates=(tnewRo*cos(0.5*rotangle*todegree),y2/2.0,-tnewRo*sin(0.5*rotangle*todegree))
      #########################################################
                hoop_xiedegree.append(face_coordinate_xiedegree[0])#存储环向层过渡区域的坐标,2021.8.31
                windtype_count+=1#2021.9.8记数缠绕形式
        ...
        cankaolunkuo=zip(x_coordinate,y_coordinate)
        R_neilunkuo=cankaolunkuo
        solvek=zip(x_coordinate,y_coordinate)
        #y2=L+xiedegree
        #y2=L+siglethick/tan(xiedegree*todegree)
        #rightAcc=y2
        # thicklist,r0,R0,alpha=sanciyangtiaohouduyuce(x_coordinate,b_fiber,tP,nR)#疑问
        # luoxuanhoudu=thicklist[-1]
        ...
        R_helixThickness_FrictionCount = 0#2021.3.26
        R_reamingThickness_FrictionCount = 0
        R_helix_slipcoefficientCount = 0
        R_reamingRadiusCount = 0
        ...
        #b_thick=0.3
    ################2021.8,18
        R_buqiangangle_count=0
        R_typeReinforce_count=0
        R_fiberwidth_count=0
    ###########2021.8.27
        R_puceng=[]    
        b=R_ellipticalheight
        #####2021.9.7
        R_hoop_transition_count=0
        windtype_count=0#2021.9.8记数缠绕形式
        
        for windtype in R_tables:
            if windtype == 'helix':
                if R_hoop_transition_count==0:#2021.9.7增加可变的环向过渡长度
                    xiedegree=1
                    y2=L+xiedegree            
                    rightAcc=y2
                else:
                    xiedegree=R_hoop_transition[R_hoop_transition_count-1]
                    y2=L+xiedegree
                    rightAcc=y2
                b_fiber=R_fiberwidth[R_fiberwidth_count]
                R_fiberwidth_count+=1
                fengtoui=cankaolunkuo#内轮廓等于参考轮廓
                #更新实际坐标
                x_coordinate=[]
                y_coordinate=[]
                ...
                #筒身
                newRi=R_thickness  #R_thickness是全局的，下一层会用这个作为初始起点
                R_thickness+=tP*2
                newRo=newRi+tP*2
                pentagon=0#五边形，用于判断螺旋层过渡区域是否需要切分
                if windtype_count==0:
                    ...
                    if R_tables[windtype_count-1]!='hoop':
                        line1=s1.Line(point1=(newRi,y2), point2=(newRi,0))
                        line2=s1.Line(point1=(newRo,y2), point2=(newRo,0))
                        line1=s1.Line(point1=(newRi,y2), point2=(fengtoui[-1][0],fengtoui[-1][1]))
                        line2=s1.Line(point1=(newRo,y2), point2=(fengtouo[-1][0],fengtouo[-1][1]))
                ...
                ######2021.8.30
                ds=p1.datums
                PL2=p1.DatumPlaneByPrincipalPlane(principalPlane=XZPLANE, offset=rightAcc)
                c = p1.cells
                p1.PartitionCellByDatumPlane(cells=c,datumPlane=ds[PL2.id])
                ######################
                f_all=p1.faces
                face_coordinate=[]
                face_coordinate_xiedegree=[]
                ...
                for k in face_coordinate_xiedegree:
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='R_set-'+str(R_lxcount)+'-'+str(count)+'-xiedegree',cells=pickcells)
                r_all_angle.append(list_angle)
                r_all_count.append(count)
                #判断是否为五边形，进行切分
                if pentagon==1:
                    ds=p1.datums
                    PL2_1=p1.DatumPlaneByPrincipalPlane(principalPlane=XZPLANE, offset=hoop_point1[1])
                    c = p1.cells
                    p1.PartitionCellByDatumPlane(cells=c,datumPlane=ds[PL2_1.id])
                #######2021.8.27
                center=(0,0,0)
                cs1=luoxuanceng.cells
                c1=cs1.getByBoundingSphere(center=center,radius=4*abs(L))
                ...
                r=R_reamingRadius[R_reamingRadiusCount]
                R_reamingRadiusCount+=1
                indexnum = 0
                #判定扩孔的位置
                for i in range(len(kfengtoui)-1):
                    a_1=kfengtoui[i][0]
                ...
                for i in range(len(fengtoui)):
                    x_coordinate.append(fengtoui[i][0])
                    y_coordinate.append(fengtoui[i][1])
                tP = R_helixThickness_Friction[R_helixThickness_FrictionCount]
                R_helixThickness_FrictionCount+=1
                thick_list,r0,R,alpha=sanciyangtiaohouduyuce(x_coordinate,b_fiber,tP,nR)
                s1 = m.ConstrainedSketch(name='R_helixLayer'+str(R_lxcount-R_buqiang), sheetSize=200.0)
                cline=s1.ConstructionLine((0,10),(0,-10))
                s1.assignCenterline(line=cline)
                ...
                #筒身
                newRi=R_thickness  # R_thickness=R0
                R_thickness+=tP*2
                newRo=newRi+tP*2
                pentagon=0#五边形，用于判断螺旋层过渡区域是否需要切分
                if windtype_count==0:
                ...
                r0=min(x_coordinate)
                h=(b**2-b**2*r0**2/R0**2)**0.5 #极孔高度
                u=R_helix_slipcoefficient[R_helix_slipcoefficientCount]#滑移系数
                R_helix_slipcoefficientCount+=1
            ...
            elif windtype == 'helixReaming(Friction)':  #2021.3.26
                if R_hoop_transition_count==0:#2021.9.7增加可变的环向过渡长度
                    xiedegree=1
                    y2=L+xiedegree           
                    rightAcc=y2
                ...
                tP = R_reamingThickness_Friction[R_reamingThickness_FrictionCount]#索引缠绕层某一序列对应的厚度。
                R_reamingThickness_FrictionCount+=1
                ...
                type_reinforce=R_strengthtype[R_buqiang]
                buqiangangle = R_strengthangle[R_buqiang]
                ...
                if type_reinforce == "windReinforce":
                    ...
                    b_fiber = R_strengthfiber[R_strengthThicknessCount]
                    R_strengthThicknessCount+=1
                    thick_list,r0,alpha= strengtheningThick(x_coordinate,b_fiber,tP,nR,Rs)

                else:
                    thick_list = [R_strengthThickness[R_strengthThicknessCount]]*len(fengtoui)
                    R_strengthThicknessCount+=1
                ...
                for i in range(len(fengtouo)-1):
                    side1Faces11 = f.findAt(((((fengtouo[i][0]+fengtouo[i+1][0])/2)*cos(0.5*rotangle*todegree),(fengtouo[i][1]+fengtouo[i+1][1])/2,-(fengtouo[i][0]+fengtouo[i+1][0])/2*sin(0.5*rotangle*todegree)), ))
                    p1.Surface(side1Faces=side1Faces11, name='R_surfStrengtheningTop'+str(R_lxcount)+'-'+str(i+2))
                ...
                f1 = p1.faces
                # 左边尖角
                pickface = f1.findAt(((0.5 * (fengtouo[0][0] + left_point[0]) * cos(
                    0.5 * rotangle * todegree),
                                       0.5 * (fengtouo[0][1] + left_point[1]),
                                       -0.5 * (fengtouo[0][0] + left_point[0]) * sin(
                                           0.5 * rotangle * todegree)),))
                p1.Surface(side1Faces=pickface,
                           name='R_surfStrengtheningTop' + str(R_lxcount) + '-' + str(
                               1))
                # 右边尖角
                pickface = f1.findAt(((0.5 * (fengtouo[-1][0] + fengtoui[-1][0]) * cos(
                    0.5 * rotangle * todegree),
                ...
                xiedegree=R_hoop_transition[R_hoop_transition_count]
                R_hoop_transition_count+=1
                y2=L+xiedegree
                rightAcc=y2
                ...
                for k in face_coordinate_xiedegree:
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='R_set-'+str(R_hxcount)+'-xiedegree',cells=pickcells)
                ...
                c1=cs1.getByBoundingSphere(center=center,radius=4*abs(L))
                R_puceng.append(c1)
                # hoop_xiedegree.append(c.findAt(face_coordinate_xiedegree[0],))
                hoop_xiedegree.append(face_coordinate_xiedegree[0])#存储环向层过渡区域的坐标,2021.8.31
                windtype_count+=1#2021.9.8记数缠绕形式
    ##########2021.8.27创建每层的集合    
        a=m.rootAssembly
        for i in range(len(L_puceng)):
            for j in range(len(R_puceng)):
                if i==j:
                    a.Set(name='ply'+str(i),cells=L_puceng[i]+R_puceng[j])
        ...
        a1 = m.rootAssembly#将装配中的部件合并为一个部件
        if len(all_instances)> 1 :
            ...
            raise AbaqusException, msg
        #创建环向过渡区域的集合2021.9.2    
        if len(hoop_xiedegree)==0:
            pass
        else:
            hoop_xiedegree_cells=p2.cells.findAt(((hoop_xiedegree[0]),))
            for i in range(1,len(hoop_xiedegree)):
                hoop_xiedegree_cells+=p2.cells.findAt(((hoop_xiedegree[i]),))
            hoop_xiedegree_set=p2.Set(name='hoop_xiedegree_set',cells=hoop_xiedegree_cells)
    ...
    ###########删除组合前的部件
        ...
        L_hxcount=0
        L_buqiang=0
        L_materialnameCount=0#2021.8.8
        
        for windtype in L_tables:
            if windtype == 'helix':
                compositeName=L_materialname[L_materialnameCount]
                L_materialnameCount+=1
                L_lxcount=L_lxcount+1
                scount=all_count[L_lxcount-1]#all_count和all_angle作用一样，是个二维列表。            
                ...
                #(环向层)过渡区域采用指派方法赋予角度
                layupOrientation = None
                region1 = p2.sets['L_set-'+str(L_lxcount)+'-'+str(scount)+'-xiedegree']
                normalAxisRegion = p2.surfaces['L_surfCylinderTop'+str(L_lxcount)]
                compositeLayup = p2.CompositeLayup(
                    name='L_helix-'+str(L_lxcount)+'-cylinder'+'-xiedegree', description='', elementType=SOLID,
                    symmetric=False)
                compositeLayup.Section(preIntegrate=OFF, integrationRule=SIMPSON,
                    poissonDefinition=DEFAULT, thicknessModulus=None, temperature=GRADIENT,
                    useDensity=OFF)
                ...
                compositeLayup.ReferenceOrientation(orientationType=DISCRETE, localCsys=None,
                    ...
                    primaryAxisDirection=AXIS_1, flipPrimaryDirection=False)
                #(环向层)过渡区域采用指派方法赋予角度
                region1 = p2.sets['L_set-'+str(L_hxcount)+'-xiedegree']
                p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    offsetType=MIDDLE_SURFACE, offsetField='', 
                    thicknessAssignment=FROM_SECTION)
                normalAxisRegion = p2.surfaces['L_surfHoopTop'+str(L_hxcount)]
                p2.MaterialOrientation(region=region1, 
                    ...
                    additionalRotationField='', angle=-90, stackDirection=STACK_3)
                normalAxisRegion = p2.surfaces['L_surfHoopTop'+str(L_hxcount)]
                p2.MaterialOrientation(region=region1, 
                    orientationType=DISCRETE, axis=AXIS_3, normalAxisDefinition=SURFACE, 
                    normalAxisRegion=normalAxisRegion, flipNormalDirection=False, 
        ...
        R_hxcount=0
        R_buqiang=0
        R_materialnameCount=0
        
        for windtype in R_tables:
            if windtype == 'helix':
                compositeName=R_materialname[R_materialnameCount]
                R_materialnameCount+=1
                R_lxcount=R_lxcount+1
                ...
                #(环向层)过渡区域采用指派方法赋予角度
                layupOrientation = None
                region1 = p2.sets['R_set-'+str(R_lxcount)+'-'+str(scount)+'-xiedegree']
                normalAxisRegion = p2.surfaces['R_surfCylinderTop'+str(R_lxcount)]
                compositeLayup = p2.CompositeLayup(
                    name='R_helix-'+str(R_lxcount)+'-cylinder'+'-xiedegree', description='', elementType=SOLID,
                    symmetric=False)
                compositeLayup.Section(preIntegrate=OFF, integrationRule=SIMPSON,
                    poissonDefinition=DEFAULT, thicknessModulus=None, temperature=GRADIENT,
                    useDensity=OFF)
                ...
                compositeLayup.ReferenceOrientation(orientationType=DISCRETE, localCsys=None,
                    ...
                    primaryAxisDirection=AXIS_1, flipPrimaryDirection=False)
                #(环向层)过渡区域采用指派方法赋予角度
                region1 = p2.sets['R_set-'+str(R_hxcount)+'-xiedegree']
                p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    offsetType=MIDDLE_SURFACE, offsetField='', 
                    thicknessAssignment=FROM_SECTION)
                normalAxisRegion = p2.surfaces['R_surfHoopTop'+str(R_hxcount)]
        ...
        #2021.8.31对区域赋予网格属性
        center=(0,0,0)
        region1=c.getByBoundingSphere(center=center,radius=4*abs(L))
        p2.setMeshControls(regions=region1, technique=STRUCTURED)
        
        #问题报错，脚本可以运行，但是插件不能运行
        # for i in range(len(hoop_xiedegree)):
            # region2=(c.findAt(hoop_xiedegree[i]),)
            # p1.setMeshControls(regions=region2,elemShape=WEDGE)
            
        #p2.setMeshControls(regions=region2,elemShape=WEDGE, technique=SWEEP)
        
        #hoop_xiedegree_cells=p2.cells.findAt(((hoop_xiedegree[0]),))
        for i in range(len(hoop_xiedegree)):
            #hoop_xiedegree_cells+=p2.cells.findAt(((hoop_xiedegree[i]),))
            region2=(c.findAt(hoop_xiedegree[i]),)
            p2.setMeshControls(regions=region2,elemShape=HEX_DOMINATED, technique=SWEEP)
        ...
        all_composite=p2.Set(name='all_composite',cells=c)
        p2.setElementType(regions=all_composite,elemTypes=(elemType1,elemType2))
        
        
        for edge in e1:
            zu=edge.getVertices()#zu类型是元组 zu=(21,19)这里并不是坐标
            zu1=ii[zu[0]].pointOn#zu1=((90.118649, 230.084771, 0.0),) 或者zu1=ii[zu[0]].pointOn[0]
            zu2=ii[zu[1]].pointOn
            p1x,p1y,p1z,p2x,p2y,p2z=zu1[0][0],zu1[0][1],zu1[0][2],zu2[0][0],zu2[0][1],zu2[0][2]
            distance=((p1x-p2x)**2+(p1y-p2y)**2+(p1z-p2z)**2)**0.2
            if zu1[0][1]==zu2[0][1] and zu1[0][2]*zu2[0][2]==0 and zu1[0][2]+zu2[0][2]!=0:#为了便于环向切分，找到环向的平行圆上的线
                meshEdgeCount-=1
                sd=edge.pointOn
        ...
        # region2=hoop_xiedegree[0]
        # for i in range(1,len(hoop_xiedegree)):
            # region2+=hoop_xiedegree[i]
        # p2.setMeshControls(regions=region2,elemShape=WEDGE, technique=SWEEP)
    ...
    ################2021.8,18
        ...
        for windtype in L_tables:
            if windtype == 'helix':
                ...
                downface_point=[]
                for i in range((len(fengtoui)-1)):
                    downface_point.append(((fengtoui[i][0]+fengtoui[i+1][0])/2.0+0.001,(fengtoui[i][1]+fengtoui[i+1][1])/2.0+0.001,0))
                for i in range((len(fengtouo)-1)):
                    upface_point.append(((fengtouo[i][0]+fengtouo[i+1][0])/2.0-0.001,(fengtouo[i][1]+fengtouo[i+1][1])/2.0-0.001,0))
                count=0
                for k in upface_point:
                    count=count+1
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
        ...
        # siglethick=hoopSingleThick#环向层单层厚度
                ...
                pickface = f.findAt(((0.5*(fengtouo[0][0]+left_point[0])*cos(0.5*rotangle*todegree),
                                   0.5*(fengtouo[0][1]+left_point[1]),
                                   -0.5*(fengtouo[0][0]+left_point[0])*sin(0.5*rotangle*todegree)),))
                p1.Surface(side1Faces=pickface,
                           name='L_surfHeadTop' + str(L_lxcount) + str(-1))
                
                for i in range(len(fengtouo)-1):
                    side1Faces11 = f.findAt(((((fengtouo[i][0]+fengtouo[i+1][0])/2)*cos(0.5*rotangle*todegree),(fengtouo[i][1]+fengtouo[i+1][1])/2,-(fengtouo[i][0]+fengtouo[i+1][0])/2*sin(0.5*rotangle*todegree)), ))
                    p1.Surface(side1Faces=side1Faces11, name='L_surfHeadTop'+str(L_lxcount)+'-'+str(i+2))
                ...
                #!
                c=p1.cells
                pickedEdges =(e1.findAt(coordinates=(((fengtoui[0][0]+fengtouo[0][0])/2.0+left_point[0])/2.0,((fengtoui[0][1]+fengtouo[0][1])/2.0+left_point[1])/2.0, 0.0)),)#这些线事先已经连上，才能用findat找到。
                p1.PartitionCellBySweepEdge(sweepPath=e1.findAt(coordinates=(fengtouo[-1][0]*cos(0.5*rotangle*todegree), L, -1*fengtouo[-1][0]*sin(0.5*rotangle*todegree))), cells=c, edges=pickedEdges)
                ...
                upface_point=[]
                downface_point=[]
                downface_point.append(((fengtoui[0][0]+left_point[0])/2.0+0.001,(fengtoui[0][1]+left_point[1])/2.0+0.001,0))
                upface_point.append(((fengtouo[0][0]+left_point[0])/2.0-0.001,(fengtouo[0][1]+left_point[1])/2.0-0.001,0))
                for i in range((len(fengtoui)-1)):
                    downface_point.append(((fengtoui[i][0]+fengtoui[i+1][0])/2.0+0.001,(fengtoui[i][1]+fengtoui[i+1][1])/2.0+0.001,0))
                for i in range((len(fengtouo)-1)):
                    upface_point.append(((fengtouo[i][0]+fengtouo[i+1][0])/2.0-0.001,(fengtouo[i][1]+fengtouo[i+1][1])/2.0-0.001,0))
        ...
        for windtype in R_tables:
            if windtype == 'helix':
                ...
                for k in upface_point:
                    count=count+1
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='R_set-'+str(R_lxcount)+'-'+str(count)+'up',cells=pickcells)
                    
                count=0 
                for k in downface_point:
                    count=count+1
                    pickcells=c.findAt(((k[0], k[1], k[2]),))
                    p1.Set(name='R_set-'+str(R_lxcount)+'-'+str(count)+'down',cells=pickcells)
                ...
                pickface = f.findAt(((0.5*(fengtouo[0][0]+left_point[0])*cos(0.5*rotangle*todegree),
                                   ...
                                   -0.5*(fengtouo[0][0]+left_point[0])*sin(0.5*rotangle*todegree)),))
                p1.Surface(side1Faces=pickface,
                           name='R_surfHeadTop' + str(R_lxcount) + str(-1))
                
                for i in range(len(fengtouo)-1):
                    side1Faces11 = f.findAt(((((fengtouo[i][0]+fengtouo[i+1][0])/2)*cos(0.5*rotangle*todegree),(fengtouo[i][1]+fengtouo[i+1][1])/2,-(fengtouo[i][0]+fengtouo[i+1][0])/2*sin(0.5*rotangle*todegree)), ))
                    p1.Surface(side1Faces=side1Faces11, name='R_surfHeadTop'+str(R_lxcount)+'-'+str(i+2))
    ...
    ###########删除组合前的部件
        ...
        for windtype in L_tables:
            if windtype == 'helix':
                ...
                list_angle=all_angle[L_lxcount-1]           
                for k in range(1,scount):
                    region1 = p2.sets['L_set-'+str(L_lxcount)+'-'+str(k)+'up']
                    p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                        offsetType=MIDDLE_SURFACE, offsetField='', 
                        thicknessAssignment=FROM_SECTION)
                    normalAxisRegion = p2.surfaces['L_surfHeadTop'+str(L_lxcount)+'-'+str(k)]
                    p2.MaterialOrientation(region=region1, 
                        ...
                        primaryAxisVector=(0.0, 1.0, 0.0), primaryAxisDirection=AXIS_1, 
                        flipPrimaryDirection=False, additionalRotationType=ROTATION_ANGLE, 
                        additionalRotationField='', angle=list_angle[k-1], stackDirection=STACK_3)
                    
                    region2=p2.sets['L_set-'+str(L_lxcount)+'-'+str(k)+'down']
                    p2.SectionAssignment(region=region2, sectionName='Section-'+str(compositeName), offset=0.0, 
                        offsetType=MIDDLE_SURFACE, offsetField='', 
                        thicknessAssignment=FROM_SECTION)
                    normalAxisRegion = p2.surfaces['L_surfHeadTop'+str(L_lxcount)+'-'+str(k)]                
                    p2.MaterialOrientation(region=region2, 
                        orientationType=DISCRETE, axis=AXIS_3, normalAxisDefinition=SURFACE, 
                        normalAxisRegion=normalAxisRegion, flipNormalDirection=False, 
                        normalAxisDirection=AXIS_3, primaryAxisDefinition=VECTOR, 
                        primaryAxisVector=(0.0, 1.0, 0.0), primaryAxisDirection=AXIS_1, 
                        flipPrimaryDirection=False, additionalRotationType=ROTATION_ANGLE, 
                        additionalRotationField='', angle=-1*list_angle[k-1], stackDirection=STACK_3)
                #筒身段赋予角度
                region1 = p2.sets['L_set-'+str(L_lxcount)+'-'+str(scount)]
                p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    offsetType=MIDDLE_SURFACE, offsetField='', 
                ...
                p2.MaterialOrientation(region=region1, 
                    ...
                    primaryAxisVector=(0.0, 1.0, 0.0), primaryAxisDirection=AXIS_1, 
                    flipPrimaryDirection=False, additionalRotationType=ROTATION_ANGLE, 
                    additionalRotationField='', angle=list_angle[-1], stackDirection=STACK_3)
                normalAxisRegion = p2.surfaces['L_surfCylinderTop'+str(L_lxcount)]
                p2.MaterialOrientation(region=region1, 
                    orientationType=DISCRETE, axis=AXIS_3, normalAxisDefinition=SURFACE, 
                    normalAxisRegion=normalAxisRegion, flipNormalDirection=False, 
                    ...
                    primaryAxisVector=(0.0, 1.0, 0.0), primaryAxisDirection=AXIS_1, 
                    flipPrimaryDirection=False, additionalRotationType=ROTATION_ANGLE, 
                    additionalRotationField='', angle=-1*list_angle[-1], stackDirection=STACK_3)
                #(环向层)过渡区域采用指派方法赋予角度
                region1 = p2.sets['L_set-'+str(L_lxcount)+'-'+str(scount)+'-xiedegree']
                p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    offsetType=MIDDLE_SURFACE, offsetField='', 
                ...
                # region1 = p2.sets['L_set-'+str(L_lxcount)+'-'+str(0)+'up']
                # p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    # offsetType=MIDDLE_SURFACE, offsetField='', 
                    # thicknessAssignment=FROM_SECTION)
                # normalAxisRegion = p2.surfaces['L_surfHeadTop'+str(L_lxcount)+'-'+str(1)]
                # p2.MaterialOrientation(region=region1, 
                    # orientationType=DISCRETE, axis=AXIS_3, normalAxisDefinition=SURFACE, 
                    # normalAxisRegion=normalAxisRegion, flipNormalDirection=False, 
                    # normalAxisDirection=AXIS_3, primaryAxisDefinition=VECTOR, 
                    # primaryAxisVector=(0.0, 1.0, 0.0), primaryAxisDirection=AXIS_1, 
                    # flipPrimaryDirection=False, additionalRotationType=ROTATION_ANGLE, 
                    # additionalRotationField='', angle=list_angle[0], stackDirection=STACK_3)
                
                # region2=p2.sets['L_set-'+str(L_lxcount)+'-'+str(0)+'down']
                # p2.SectionAssignment(region=region2, sectionName='Section-'+str(compositeName), offset=0.0, 
                    # offsetType=MIDDLE_SURFACE, offsetField='', 
                    # thicknessAssignment=FROM_SECTION)
                # normalAxisRegion = p2.surfaces['L_surfHeadTop'+str(L_lxcount)+'-'+str(1)]                
                # p2.MaterialOrientation(region=region2, 
                    ...
                    # primaryAxisVector=(0.0, 1.0, 0.0), primaryAxisDirection=AXIS_1, 
                    # flipPrimaryDirection=False, additionalRotationType=ROTATION_ANGLE, 
                    # additionalRotationField='', angle=-1*list_angle[0], stackDirection=STACK_3)
                ...
                L_hxcount=L_hxcount+1
                #layupOrientation = None
                region1 = region1 = p2.sets['L_set-'+str(L_hxcount)]
                p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    offsetType=MIDDLE_SURFACE, offsetField='', 
                    thicknessAssignment=FROM_SECTION)
                normalAxisRegion = p2.surfaces['L_surfHoopTop'+str(L_hxcount)]
        ...
        for windtype in R_tables:
            if windtype == 'helix':
                ...
                list_angle=r_all_angle[R_lxcount-1]
                for k in range(1,scount):
                    region1 = p2.sets['R_set-'+str(R_lxcount)+'-'+str(k)+'up']
                    p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                        offsetType=MIDDLE_SURFACE, offsetField='', 
                        thicknessAssignment=FROM_SECTION)
                    normalAxisRegion = p2.surfaces['R_surfHeadTop'+str(R_lxcount)+'-'+str(k)]
                    ...
                    region2=p2.sets['R_set-'+str(R_lxcount)+'-'+str(k)+'down']
                    p2.SectionAssignment(region=region2, sectionName='Section-'+str(compositeName), offset=0.0, 
                        offsetType=MIDDLE_SURFACE, offsetField='', 
                        thicknessAssignment=FROM_SECTION)
                    normalAxisRegion = p2.surfaces['R_surfHeadTop'+str(R_lxcount)+'-'+str(k)]                
                ...
                # region1 = p2.sets['R_set-'+str(R_lxcount)+'-'+str(0)+'up']
                # p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    # offsetType=MIDDLE_SURFACE, offsetField='', 
                    # thicknessAssignment=FROM_SECTION)
                # normalAxisRegion = p2.surfaces['R_surfHeadTop'+str(R_lxcount)+'-'+str(1)]
                # p2.MaterialOrientation(region=region1, 
                    # orientationType=DISCRETE, axis=AXIS_3, normalAxisDefinition=SURFACE, 
                    # normalAxisRegion=normalAxisRegion, flipNormalDirection=False, 
                    # normalAxisDirection=AXIS_3, primaryAxisDefinition=VECTOR, 
                ...
                # region2=p2.sets['R_set-'+str(R_lxcount)+'-'+str(0)+'down']
                # p2.SectionAssignment(region=region2, sectionName='Section-'+str(compositeName), offset=0.0, 
                    # offsetType=MIDDLE_SURFACE, offsetField='', 
                    # thicknessAssignment=FROM_SECTION)
                # normalAxisRegion = p2.surfaces['R_surfHeadTop'+str(R_lxcount)+'-'+str(1)]                
                ...
                R_hxcount=R_hxcount+1
                layupOrientation = None
                region1 = region1 = p2.sets['R_set-'+str(R_hxcount)]
                p2.SectionAssignment(region=region1, sectionName='Section-'+str(compositeName), offset=0.0, 
                    offsetType=MIDDLE_SURFACE, offsetField='', 
                    thicknessAssignment=FROM_SECTION)
                normalAxisRegion = p2.surfaces['R_surfHoopTop'+str(R_hxcount)]
        ...
        center=(0,0,0)
        region1=c.getByBoundingSphere(center=center,radius=4*abs(L))
        p2.setMeshControls(regions=region1, elemShape=HEX_DOMINATED, technique=SWEEP)
    ...
    mdb.models[modelName].StaticStep(name='Step-1', previous='Initial', 
        maxNumInc=1000, initialInc=0.1, maxInc=0.1, nlgeom=ON)
    session.viewports['Viewport: 1'].assemblyDisplay.setValues(step='Step-1')

    #相互作用，循环对称,需要将内衬和复合材料的一个侧面定义为一个集合，需要将它们合并
    modelName=modelName
    m = mdb.models[modelName]
    ...
    s2=a.instances[assemblyName].faces
    for face in s1:
        centercoordinate = face.getCentroid() # 将曲面一个一个的输入判断，#centercoordinate=((1134.46346, 120.951893, 0.0),)
        if centercoordinate[0][2] == 0.0:
            Masterface1.append(centercoordinate)

    for face in s2:
        centercoordinate = face.pointOn  # 将曲面一个一个的输入判断，#centercoordinate=((1134.46346, 120.951893, 0.0),)
        if centercoordinate[0][2] == 0.0:
            Masterface2.append(centercoordinate)
    Masterface=Masterface1+Masterface2
    Slaveface=[]
    Slaveface1=[]
    Slaveface2=[]
    for i in range(len(Masterface1)):
        abc1=Masterface1[i][0][0]*math.cos(math.pi*rotangle/180)
        abc2=Masterface1[i][0][1]
        abc3=-Masterface1[i][0][0]*math.sin(math.pi*rotangle/180)
        Slaveface1.append(((abc1,abc2,abc3), ))
    for i in range(len(Masterface2)):
        abc1=Masterface2[i][0][0]*math.cos(math.pi*rotangle/180)
        abc2=Masterface2[i][0][1]
        abc3=-Masterface2[i][0][0]*math.sin(math.pi*rotangle/180)
        Slaveface2.append(((abc1,abc2,abc3), ))
    Slaveface=Slaveface1+Slaveface2

    side1Faces1=s1.findAt(Masterface1[0])
    for i in range(1,len(Masterface1)):
        faces1=s1.findAt(Masterface1[i])
        side1Faces1+=faces1#遍历读取面
    for i in range(len(Masterface2)):
        faces1=s2.findAt(Masterface2[i])
        side1Faces1+=faces1
    regionMaterface=a.Surface(side1Faces=side1Faces1, name='m_Surf-1')


    side1Faces2=s1.findAt(Slaveface1[0])
    for i in range(1,len(Slaveface1)):
        faces2 = s1.findAt(Slaveface1[i])
        side1Faces2+=faces2#遍历读取面
    for i in range(len(Slaveface2)):
        faces2 = s2.findAt(Slaveface2[i])
        side1Faces2+=faces2#遍历读取面
    ...
    r1 = a.ReferencePoint(point=(0.0, 10, 0.0))
    ...
    # 建立循环对称相互作用
    mdb.models[modelName].CyclicSymmetry(name='Int-1', createStepName='Initial',
        ...
        adjustTie=True, repetitiveSectors=360/int(rotangle),
        extractedNodalDiameter=ALL_NODAL_DIAMETER, excitationNodalDiameter=0)
    #: The interaction "Int-1" has been created.
    #建立局部柱坐标系
    DatumcsysCylinder=a.DatumCsysByThreePoints(point1=(newRo*cos(0.5*rotangle*todegree),0, -newRo*sin(0.5*rotangle*todegree)), 
    name='Datum csys-Cylinder1',
    ...
    # #L_neichenface
    # #L_fucaiface
    # L_fucaiface=s1.findAt(((((L_neilunkuo[0][0]+L_neilunkuo[1][0])/2)*cos(0.5*rotangle*todegree),((L_neilunkuo[0][1]+L_neilunkuo[1][1])/2),-((L_neilunkuo[0][0]+L_neilunkuo[1][0])/2)*sin(0.5*rotangle*todegree)),))
    # for i in range(len(L_neilunkuo)-1):
        # L_fucaiface1=s1.findAt(((((L_neilunkuo[i][0]+L_neilunkuo[i+1][0])/2)*cos(0.5*rotangle*todegree),((L_neilunkuo[i][1]+L_neilunkuo[i+1][1])/2),-((L_neilunkuo[i][0]+L_neilunkuo[i+1][0])/2)*sin(0.5*rotangle*todegree)),))
        # L_fucaiface+=L_fucaiface1
    # L_fucaiface2=s1.findAt(((L_neilunkuo[-1][0]*cos(0.5*rotangle*todegree),leftAcc+xiedegree/2,-L_neilunkuo[-1][0]*sin(0.5*rotangle*todegree)),))
    # L_fucaiface+=L_fucaiface2
    # L_fucaiface3=s1.findAt(((L_neilunkuo[-1][0]*cos(0.5*rotangle*todegree),leftAcc-xiedegree/2,-L_neilunkuo[-1][0]*sin(0.5*rotangle*todegree)),))
    # L_fucaiface+=L_fucaiface3
    
    # R_fucaiface=s1.findAt(((((R_neilunkuo[0][0]+R_neilunkuo[1][0])/2)*cos(0.5*rotangle*todegree),((R_neilunkuo[0][1]+R_neilunkuo[1][1])/2),-((R_neilunkuo[0][0]+R_neilunkuo[1][0])/2)*sin(0.5*rotangle*todegree)),))
    # for i in range(len(R_neilunkuo)-1):
        # R_fucaiface1=s1.findAt(((((R_neilunkuo[i][0]+R_neilunkuo[i+1][0])/2)*cos(0.5*rotangle*todegree),((R_neilunkuo[i][1]+R_neilunkuo[i+1][1])/2),-((R_neilunkuo[i][0]+R_neilunkuo[i+1][0])/2)*sin(0.5*rotangle*todegree)),))
        # R_fucaiface+=R_fucaiface1
    # R_fucaiface2=s1.findAt(((R_neilunkuo[-1][0]*cos(0.5*rotangle*todegree),rightAcc+xiedegree/2,-R_neilunkuo[-1][0]*sin(0.5*rotangle*todegree)),))
    # R_fucaiface+=R_fucaiface2
    # R_fucaiface3=s1.findAt(((R_neilunkuo[-1][0]*cos(0.5*rotangle*todegree),rightAcc-xiedegree/2,-R_neilunkuo[-1][0]*sin(0.5*rotangle*todegree)),))
    # R_fucaiface+=R_fucaiface3
    
    # fucaiface=L_fucaiface+R_fucaiface
    
    # region1=a.Surface(side1Faces=fucaiface, name='s_fucaiface')

    # L_neichenface=s2.findAt(((L_neilunkuo[2][0]*cos(0.5*rotangle*todegree),L_neilunkuo[2][1],-L_neilunkuo[2][0]*sin(0.5*rotangle*todegree)),))
    # L_neichenface1=s2.findAt(((L_neilunkuo[-1][0]*cos(0.5*rotangle*todegree),leftAcc,-L_neilunkuo[-1][0]*sin(0.5*rotangle*todegree)),))
    # L_neichenface+=L_neichenface1
    
    # R_neichenface=s2.findAt(((R_neilunkuo[2][0]*cos(0.5*rotangle*todegree),R_neilunkuo[2][1],-R_neilunkuo[2][0]*sin(0.5*rotangle*todegree)),))
    # R_neichenface1=s2.findAt(((R_neilunkuo[-1][0]*cos(0.5*rotangle*todegree),rightAcc,-R_neilunkuo[-1][0]*sin(0.5*rotangle*todegree)),))
    # R_neichenface+=R_neichenface1
    
    # neichenface=L_neichenface+R_neichenface
    
    # region2=a.Surface(side1Faces=neichenface, name='m_neichenface')

    # mdb.models['Model-2'].Tie(name='Constraint-1', master=region2, slave=region1, 
